...
private case class NewLassoCalculator(data: DenseMatrix[Double], outputs: DenseVector[Double], lambda: Double, workArray: Array[Double], MAX_ITER: Int=100, IMPROVE_THRESHOLD: Double=1e-8) {
  /*
  ...
  lazy val result: NewLassoResult = {
    ...
    NewLassoResult(resultVec, computeRsquared, lambda)
  }
...
case class NewLassoResult(coefficients: DenseVector[Double], rSquared: Double, lambda: Double) extends RegressionResult[DenseVector[Double], Double] {
  require(lambda >= 0)
  def apply(x: DenseVector[Double]): Double = coefficients.dot(x)
}

object newLasso extends UFunc {
  /*
   ...
   */
  implicit val matrixVectorWithWorkArray: Impl4[DenseMatrix[Double], DenseVector[Double], Double, Array[Double], NewLassoResult] = new Impl4[DenseMatrix[Double], DenseVector[Double], Double, Array[Double], NewLassoResult] {
    def apply(data: DenseMatrix[Double], outputs: DenseVector[Double], lambda: Double, workArray: Array[Double]): NewLassoResult = NewLassoCalculator(data, outputs, lambda, workArray).result
  }

  implicit val matrixVectorSpecifiedWork: Impl4[DenseMatrix[Double], DenseVector[Double], Double, Int, NewLassoResult] = new Impl4[DenseMatrix[Double], DenseVector[Double], Double, Int, NewLassoResult] {
    def apply(data: DenseMatrix[Double], outputs: DenseVector[Double], lambda: Double, workSize: Int): NewLassoResult = NewLassoCalculator(data, outputs, lambda, new Array[Double](workSize)).result
  }

  implicit val matrixVector: Impl3[DenseMatrix[Double], DenseVector[Double], Double, NewLassoResult] = new Impl3[DenseMatrix[Double], DenseVector[Double], Double, NewLassoResult] {
    def apply(data: DenseMatrix[Double], outputs: DenseVector[Double], lambda: Double): NewLassoResult = NewLassoCalculator(data.copy, outputs.copy, lambda, new Array[Double](math.max(1, data.rows*data.cols*2))).result
  }
}
